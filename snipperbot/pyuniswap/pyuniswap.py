...
class Token:
    ...
    def create_transaction_params(self, value=0, gas_price=None, gas_limit=None,num=1):
        # if not self.is_connected():
        #     raise RuntimeError('Please connect the wallet first!')
        # print(f'nonce ----{self.web3.eth.getTransactionCount(self.wallet_address)},  {num}')
        return {
            "from": self.wallet_address,
            "value": value,
            'gasPrice': gas_price,
            "gas": self.gas_limit,
            "nonce": self.web3.eth.getTransactionCount(self.wallet_address)+num-1
        }
    ...
    @require_connected
    def buy(self, consumed_token_amount, consumed_token_address=ETH_ADDRESS, slippage=0.01, timeout=900, speed=1, nonce=1):
        ...
        rate = received_amount * pow(10,18 - decimals) /consumed_token_amount
        if rate < 1:
            return "bad token"
        ...
        if consumed_token_address == self.ETH_ADDRESS:
            ...
            params = self.create_transaction_params(value=consumed_token_amount, gas_price=gas_price,num=nonce)
        else:
            ...
            func = self.router.functions.swapExactTokensForTokens(consumed_token_amount, min_out,
                                                                  ...
                                                                  self.wallet_address, int(time.time() + timeout))
            params = self.create_transaction_params(gas_price=gas_price,num=nonce)
    ...
    @require_connected
    def buyWBNB(self, consumed_token_amount, consumed_token_address=ETH_ADDRESS, slippage=0.01, timeout=900, gas_price=5000000000, nonce = 1):
        gas_price = gas_price
        func = self.router.functions.swapExactTokensForTokens(consumed_token_amount, 0,
                                                              [consumed_token_address, self.address],
                                                              self.wallet_address, int(time.time() + timeout))
        params = self.create_transaction_params(gas_price=gas_price, num=nonce)
        return self.send_transaction(func, params)
